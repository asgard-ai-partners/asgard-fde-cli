...
| ~~P16~~ | ~~What is the URL of a given resource's page in the Console?~~ **Answered 2026-09-17.** Every shape is in `../wiki/console.md`, read off the hrefs of a partner deck built against a production Console; the workspace and pipeline ids in them were corroborated against what that engagement's `.asgard-cli.yaml` records, which is what makes them shapes rather than one page's URLs. The prefix this row was opened for is `/workspace/<workspaceId>/project/<projectId>/`. **What replaced the question is narrower and is not a platform unknown**: an engagement holds neither its platform project id nor a release id in any machine-readable place - the project id appears in `.asgard-pipeline.yaml` as a comment - so `asgard-cli links` prints the shapes it cannot fill and says which id is missing rather than guessing one | - |
