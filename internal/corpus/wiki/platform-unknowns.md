...
**Checked:** 2026-09-02, re-read 2026-09-11 against [asgard-kube](https://github.com/asgard-ai-platform/asgard-kube) `cbd8d70` and asgard-docs `f00e0ee` - each row was re-checked to confirm neither source answers it.
...
| P10 | **ANSWERED, and the answer is that there is no closed set.** asgard-docs `23409b3` (unread) (2026-09-09) publishes a page per processor under `developer-reference/processor/`, each verified against the editor's own property panel rather than only against the code - because `processors.json` "omits the section a `has_dynamic_config` processor generates, and it lists keys the editor hides". For a processor that accepts dynamic config, **the palette's list is a starting set and keys an author adds beyond it are legitimate**. That is why `ProcessorDefinitions` looked incomplete: `await` and `temperature` are dynamic config, not missing entries. So a chart still cannot be validated against a key list, and now there is a reason rather than a gap. **The pages themselves have not been read into this wiki** - see `../wiki/README.md` for what moving a citation requires |
