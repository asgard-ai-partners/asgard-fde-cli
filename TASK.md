...
| `install-scripts` `install.sh` and `install.ps1` - the two things a customer pipes into a shell from a URL, and the one surface here that no `hack` check reads at all | `install.sh` run end to end on macOS into a scratch directory against release v0.1.2: asset resolved, checksum matched by hash, binary installed and executed. Its Linux branch run the same way with the linux asset - downloaded, verified and extracted, `file` reporting a statically linked ELF - with nothing executed, because there is no Linux here. **`install.ps1` has never been run**: it is written and type-checked by nothing, PowerShell is not on this machine, and the CI job that executes it has not run yet | 2026-09-23 |
| `binary-replacement` what `asgard-cli update` does to the file it is executing from, which is the one thing here that cannot be undone by running it again | the Unix half exercised for real on macOS - a v0.0.1 install replaced with the published 0.1.2, the new binary run before the rename and the old one gone afterwards - plus the refusals for Homebrew, dpkg, `go install`, an unwritable directory and a development build. **The Windows two-rename path has never executed anywhere**: it is behind a build tag, so every local run and the Linux CI job take the Unix branch, and its own test only runs on a runner that has not run yet | 2026-09-23 |
