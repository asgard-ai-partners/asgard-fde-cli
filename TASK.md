...
| `install-scripts` `install.sh` and `install.ps1` - the two things a customer pipes into a shell from a URL, and the one surface here that no `hack` check reads at all | both run end to end against the published release, each on the platform it is for: `install.sh` on macOS, by hand into a scratch directory and again on `macos-latest`; `install.ps1` on `windows-latest`, which downloaded the zip, matched the checksum by hash, wrote the PATH entry and ran the binary. **The Linux branch is the one nothing executes** - it was driven by hand as far as extraction, `file` reporting a statically linked ELF, and no Linux runner runs it | 2026-09-23 |
| `binary-replacement` what `asgard-cli update` does to the file it is executing from, which is the one thing here that cannot be undone by running it again | the Unix half against a real install: a v0.0.1 binary replaced with the published 0.1.2, the new one run before the rename and the old one gone afterwards, plus the refusals for Homebrew, dpkg, `go install`, an unwritable directory and a development build. The Windows two-rename path and the sweep of the displaced file now run on `windows-latest`. **What no run has covered is the case the two renames exist for** - replacing a `.exe` a process is executing from. Both sides are exercised against ordinary files, and Windows only locks the running one, so the lock itself has never been met | 2026-09-23 |
