...
| the pinned enum and constraint tables against the CRDs, and **every CEL-rule count this repository states** - 231 enforced against 79 markers, which are two numbers easy to write for each other | `hack/check-tables.py` (needs `$ASGARD_KUBE`) |
