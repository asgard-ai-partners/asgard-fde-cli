...
| `scaffold-templates`  the 2,783 lines a customer repository receives that are not skills | 2026-09-14, **for its platform claims only**: the six `spec.` fields `AGENTS.md.tmpl` names are all in asgard-kube `cbd8d70`. That file is 912 lines and is the AGENTS.md every engagement works under, and **its guidance has been held against nothing** - it is the largest unread surface here now that the skills are read. `--commands`, `--links` and `--paths` reach the commands and pointers in it and nothing reaches the advice |
