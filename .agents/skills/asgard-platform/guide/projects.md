...
`requirements/requests/<request-id>-*.md`, and the audience you settled
...
Then, in this order:
  ...
  1. **Anything the interview did not settle**, one question each:
         ...
         asgard-cli question add "<question>" --blocks <request-id> --ask "<who>"
  ...
  4. **The first module of the living spec**, `docs/spec/<spec-slug>/architecture.md`:
     ...
     **Add it to the module index in `docs/spec/<spec-slug>/README.md` in the
...
**`no project yet`** and says its audience has not been decided.
    ...
    asgard-cli request target <request-id> <slug>
    asgard-cli request ready <request-id>          once the audience and the scope are settled
